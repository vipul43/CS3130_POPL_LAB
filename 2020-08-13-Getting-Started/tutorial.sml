...
val origin : real * real = (0.0, 0.0)
 ...
 (* val origin : real * real = (0.0, "0.0") *)
 (* ANS
    Raises tycon mismatch error
    pattern: real * real
    expression: real * string
    these both do not agree
  *)
...
fun curry2 f x = fn y => f (x, y)
fun curry3 f   = fn x => fn y => f (x, y)
val curry4     = fn f => fn x => fn y => f (x, y)

fun uncurry2 f = fn (x, y) => f x y
val uncurry3   = fn f => fn (x, y) => f x y
...
fun foldrev _ x [] = x
  | foldrev f x (y::ys) = f y (foldrev f x ys);
...
datatype 'a mylist = null  (* using nil is raising error *)
  | cons of ('a * 'a mylist)


(* We can define functions again by pattern matching. *)
fun isEmpty null = true
  | isEmpty _   = false

fun listLength null            = 0
